...
export default function ToDos() {
    ...
    const resetForm = () => {
        ...
        // Reset date to selectedDate or today if needed, but here we just keep current state or maybe reset to today?
        // Let's reset to selected date default
        setDate(format(selectedDate, "yyyy-MM-dd"));
        setTime("");
        setTime("");
        setRecurrence('none');
    ...
    const todosOnSelectedDate = todos.filter(t => t.due_date && isSameDay(parseISO(t.due_date), selectedDate));
    ...
    const undatedTodos = todos.filter(t => !t.due_date);
    // Sort undated by created_at (assuming created_at exists, or just by order)
    // If created_at is optional, maybe fallback to title or keeping order.

    const visibleUndatedTodos = undatedTodos.slice(0, visibleUndatedCount);
    ...
    const calendarItems = [
        ...todos.filter(t => t.due_date).map(t => ({
            ...
            name: t.title,
            type: 'task',
            exercises: [], // Satisfy Workout interface structural requirements
            duration: '0'
        })),
        ...googleEvents
            .filter(e => e.start && (e.start.date || e.start.dateTime)) // Ensure valid start object
            .map(e => ({
                id: e.id,
                ...
                <div className="w-full lg:w-80 flex-none flex flex-col gap-4 border lg:border-l-0 border-border/50 bg-card rounded-xl lg:rounded-l-none lg:bg-background lg:border-l lg:border-t-0 lg:border-b-0 lg:border-r-0 lg:shadow-none shadow-sm p-4 h-[30vh] lg:h-full min-h-0">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 scrollbar-thin">
                        ...
                        <div>
                            ...
                            {undatedTodos.length > visibleUndatedCount && (
                                <Button
                                    variant="ghost"
                                    className="w-full mt-2 text-xs text-muted-foreground hover:text-primary"
                                    onClick={() => setVisibleUndatedCount(prev => prev + 5)}
                                >
                                    Load More ({undatedTodos.length - visibleUndatedCount} remaining)
                                </Button>
                            )}
